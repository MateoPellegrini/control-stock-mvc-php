...
<div class="container mx-auto px-4 py-10">
    <div class="max-w-3xl mx-auto bg-white/10 backdrop-blur rounded-3xl border border-white/10 p-6 text-slate-100 shadow-xl">
        ...
        <form id="sale-form"
            action="index.php?controller=sales&action=store"
            method="POST"
            class="space-y-3">
                ...
                <select name="producto_id"
                    ...
                    <?php foreach ($productos as $p): ?>
                        <option
                            ...
                            data-precioventa="<?php echo (float)$p['precio_venta']; ?>"
                        >
                            <?php
            ...
            <div class="mb-3">
                ...
                <textarea name="motivo" rows="2"
                        class="form-control form-control-sm bg-slate-900/70 border-slate-600 text-slate-100"
                        placeholder="Venta mostrador, ticket Nro..."></textarea>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const select = document.querySelector('#sale-form select[name="producto_id"]');
    const precioInput = document.querySelector('#sale-form input[name="precio_unitario"]');

    if (select && precioInput) {
        select.addEventListener('change', function () {
            const option = this.selectedOptions[0];
            if (option) {
                const precioVenta = option.dataset.precioventa;
                precioInput.value = precioVenta ? precioVenta : '';
    ...
    setupTotalCalculator('#sale-form');
});
</script>
